...
private:
    ...
    // Evaluates the quadratic B-spline on the local support
...
public:
    ...
    static real evaluate(const Vector3& center, real radius, 
        ...
        // to be in [3/2, 3], then we can evaluate the B-spline.
...
private:
    ...
    // Evaluates the linear B-spline on the local support
    // [0, 2].
    static inline constexpr real evaluateBSpline(real t){
        if(t < 0.0 || t > 2.0){
            return 0;
...
public:
    ...
    static real evaluate(const Vector3& center, real radius, 
        ...
        // First we map the input, which is naturally in [0, 1]
        // to be in [1, 2], then we can evaluate the B-spline.
        return evaluateBSpline(input + 1.0);
    }

};


class ConstantBSpline{

private:

    // Evaluates the B-spline on the local support [0, 1].
    static inline constexpr real evaluateBSpline(real t){
        if(t < 0.0 || t > 1.0){
            return 0.0;
        }
